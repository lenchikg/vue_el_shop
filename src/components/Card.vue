    ...
    <BaseCard :isFullWidth="true">
      <div class="card__container">
        ...
        <div class="card__actions" v-if="!isProductInCart">
          ...
          <BaseIconButton variant="contained" text="В КОРЗИНУ" iconColor="lightgray" iconHoverColor="#ffa801" @click="addToCart">
            <CartIcon />
          </BaseIconButton>
        </div>

        <FadeTransition>
          <BaseButton @click="openModal('cart')" variant="contained" mode="success" v-if="isProductInCart">УЖЕ В КОРЗИНЕ</BaseButton>
        </FadeTransition>
      </div>
    </BaseCard>
  </div>
